...
kubectl delete secret  beda-software-registry-rgv-api
...
helm upgrade --set backend.image.tag=master --set backend.image.hash=$1  --set frontend.image.hash=$1 --namespace=bedasoftware -i rgv .
